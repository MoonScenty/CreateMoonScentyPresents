...
| 숯가마 | 원목 → 숯. 연료가 필요 없다. **타는 동안 그 자체가 열원이다** | **화섭과 부시로 점화** + 위에 덮개 |
...
**숯가마만 불을 스스로 갖는다.** 나머지 스테이션은 아래에 놓인 불을 매 틱 읽지만, 숯가마는 화섭과 부시로 한 번 치면 그때부터 제 안에서 탄다. 숯을 굽는 것은 불 위에 올려두는 일이 아니라 나무 더미에 불을 붙이고 덮어버리는 일이고, 타는 것이 곧 실려 있는 나무다.

```text
원목을 넣는다  →  화섭과 부시로 친다  →  위를 덮는다  →  탄다
                                  실린 나무가 다 되면 꺼진다
```

점화에는 덮개가 필요 없고 가동에는 필요하다. **불을 붙인 다음 묻는다** — 실제 숯가마를 돌리는 순서 그대로다. 실려 있는 나무가 떨어지면 꺼지므로 한 판마다 다시 쳐야 하고, 맨손으로 비워도 같이 꺼진다.
...
| 숯가마 | `charcoal_pit` | 원목 → 숯. 화섭과 부시로 붙이고 덮어 둔다. 실린 나무가 떨어지면 꺼진다. **타는 동안 위에 얹힌 것을 데운다** | 점토 벽돌 |
