...
**졸업 시험은 `bronze_casing` 하나다.** 그리고 그 하나에는 이 시대가 세운 것이 전부 동원된다 — 선광기가 주석을 찾고, 숯가마와 풀무가 구리를 녹이고, 파운드리가 붓고, 크랭크가 믹서를 돌린다. 마지막 행동이 시대 전체의 요약이 되도록 짜여 있고, 그래서 지름길을 하나도 두지 않았다. 자세한 것은 [브론즈 시대 진입 조건](#브론즈-시대-진입-조건)에 있다.
...
| 파운드리 베이슨 | 녹인 금속을 담는 그릇. **혼자서는 아무것도 녹이지 않는다** | — |
| 파운드리 뚜껑 | 베이슨 위에 닫아야 녹기 시작한다. 열면 멈추고, 열었을 때만 손으로 넣을 수 있다 | 베이슨 위 |
| 수도꼭지 | 붙은 탱크를 아래로 흘린다. 받을 곳이 없으면 열리지 않는다 | 유체를 가진 블록에 부착 |
| 주조대 | 틀을 올려 두고 금속을 받아 굳힌다. **틀이 요구하는 만큼만 받는다** | 위에서 부어짐 |
...
#### 붓는 것이지 들고 다니는 것이 아니다

**녹은 금속에는 버킷이 없다.** 주석도 아연도 구리도 철도 브론즈도, 통에 담아 주머니에 넣을 수 없다. 금속을 옮기는 방법은 하나뿐이다 — 베이슨 옆구리에 수도꼭지를 달고, 그 아래에 주조대를 놓고, 중력에 맡긴다.

```text
[숯가마]  →  [베이슨 + 뚜껑]  →  [수도꼭지]  →  [주조대 + 틀]  →  주괴
   불          녹인다            흘린다         굳힌다
```

이 네 칸이 파운드리의 전부이고, 하나라도 빠지면 금속은 그 자리에서 멈춘다. 버킷을 하나 주면 수도꼭지와 주조대가 동시에 장식이 되므로, 주지 않는다.

**뚜껑이 없으면 아무것도 녹지 않는다.** 녹이는 주체는 베이슨이 아니라 그 위에 닫힌 뚜껑이다. 베이슨만 놓고 불을 아무리 때도 아무 일도 일어나지 않으며, 뚜껑이 열려 있어도 마찬가지다. 여는 것이 곧 멈추는 것이고, 그것이 손을 넣을 수 있는 유일한 순간이기도 하다.

**주조대는 틀이 요구하는 만큼만 받는다.** 주괴 틀에 필요한 것이 90mB면 90mB에서 더 들어가지 않고, 수도꼭지는 거기서 스스로 멈춘다. 남는 금속이 생기지 않으므로 부어 놓고 자리를 떠도 되고, 굳은 주물은 손으로 집어 가면 그만이다.
...
| 4 | `create:mechanical_press` | `press_ram` | Shaping | `crafting/kinetics/mechanical_press` |
...
`mechanical_press`는 세로 3칸 `"S"/"C"/"I"`(축·케이싱·철 블록)이다. **칸 수는 그대로 두고, 맨 아래 철 블록을 끌로 깎아 놓는다.**

```text
 S        →      S     S = wooden_shaft
 C               C     C = create:andesite_casing
 I               R     R = press_ram      ← c:storage_blocks/iron 을 Shaping
```

`S` 자리는 `create:shaft`가 아니라 **`wooden_shaft`**다. 프레스는 석기 시대의 마지막 조립물이므로 석기 시대의 부품으로 짓는다.

**재료를 하나 더 얹지 않고, 있던 재료를 깎게 만든다.** 철 블록 하나는 Create가 원래 요구하던 것 그대로이므로 **관문 4가 철을 더 먹지 않는다.** 늘어난 것은 비용이 아니라 동작 하나 — 48틱짜리 Shaping이고, 그것이 네 도구 중 마지막인 돌 끌이 이 시대에 하는 유일한 일이다.

프레스가 케이싱만으로 도달하는 것을 막는다. **이것이 시대의 종결 조건이다.**

램은 프레스에서 실제로 내려찍는 부품이다. 시대의 마지막 조립물이 요구하는 것이 **철을 두들겨 만든 덩어리가 아니라 깎아 만든 형상**이라는 점에서, 관문 1의 망치(부수기)와 관문 4의 끌(다듬기)이 시대의 양 끝에 선다.
...
[5장] 프레스 — Shaping
Stone Chisel + 철 블록 → Press Ram
↓
wooden_shaft + 케이싱 + Press Ram → create:mechanical_press
        (철은 더 들지 않는다. 늘어난 것은 깎는 동작 하나뿐)
...
↓
크랭크 하나로 펌프와 믹서를 함께 돌려 브론즈 → 주조대에서 브론즈 주괴
↓
5장에서 세운 프레스로 주괴를 눌러 Bronze Sheet
↓
Sheet 를 들고 껍질 벗긴 원목에 우클릭 → bronze_casing
↓
브론즈 시대 (= 무인 동력 해금)
```
...
무엇이 어떻게 동원되는지는 [브론즈 시대 진입 조건](#브론즈-시대-진입-조건)에 표로 적어 두었다. **놀고 있는 스테이션이 하나도 없다는 것이 6장의 설계 조건이다.**
...
| 프레스 램 | `press_ram` | 관문 4. 프레스가 내려찍는 부품. **철 블록을 대체하므로 철이 더 들지 않는다** | Shaping: `c:storage_blocks/iron` |
| 브론즈 판 | `bronze_sheet` | 시대의 마지막 재료. 케이싱의 겉면 | Pressing: 브론즈 주괴 |
...
| 브론즈 케이싱 | `bronze_casing` | **석기 시대의 종착점.** 판을 들고 껍질 벗긴 원목에 우클릭 | Item Application: 껍질 벗긴 원목 + 브론즈 판 |
...
> **석기 시대가 세운 것 전부를 한 번씩 다시 써서 `bronze_casing` 하나를 만든다.**

주괴가 아니라 **케이싱**이 마침표다. 주괴는 재료일 뿐이고, 재료로 끝나면 시대가 닫히지 않는다 — 눌러서 판으로 만들고 그 판을 나무에 입혀야 비로소 다음 시대가 쓸 물건이 된다.

```text
브론즈 주괴  →  [프레스]  →  Bronze Sheet  →  [껍질 벗긴 원목에 우클릭]  →  bronze_casing
```

그리고 **이 마지막 세 걸음이 5장에서 세운 프레스를 처음으로 쓰는 자리다.** 프레스는 관문 4를 통과했다는 증명으로 만들어졌지만 그 자체로는 아무것도 하지 않았는데, 여기서 자기 일을 한다. 관문이 장식이 아니었다는 것이 시대의 마지막 동작에서 확인된다.

진입 조건은 **어떤 아이템 하나를 만드는 것이 아니라, 지금까지 지은 것이 전부 살아 있음을 한 번에 증명하는 것**이다. 케이싱 하나를 뽑는 동안 놀고 있는 스테이션이 하나도 없다.

케이싱을 만드는 방식은 Create 그대로다 — 판을 손에 들고 껍질 벗긴 원목에 우클릭한다(`item_application`). 안산암 케이싱을 붓으로 옮긴 것은 그것이 **관문**이라 동작 값을 매겨야 했기 때문이고, 이쪽은 **보상**이라 Create의 몸짓을 그대로 둔다.

Create 아이템을 관문으로 쓰지 않는다. 브론즈는 이 팩의 금속이고, 그 앞에 서는 것도 이 팩이 지은 라인이어야 한다.

#### 브론즈 한 판에 무엇이 동원되는가

| 세운 것 | 언제 지었나 | 브론즈에서 하는 일 |
|---|---|---|
| 손 도구 넷 | 1장 | 톱이 축을, 망치가 부스러기를, 붓이 케이싱을, 끌이 프레스 램을 만들어 여기까지 왔다 |
| 채취통·수지 | 1장 | 케이싱의 시멘트. 그것이 없으면 프레스가 없고, 프레스가 없으면 5장이 닫히지 않는다 |
| 노천 가마 | 2장 | 파운드리 한 벌이 전부 여기서 구워져 나왔다 |
| 숯가마 | 2장 | 구리를 녹이는 불. 모닥불로는 닿지 않는다 |
| 풀무 | 2장 | 숯가마를 한 칸 올린다. **그리고 축을 꿰어 공기 펌프가 된다** |
| 파운드리 베이슨·뚜껑 | 2장 | 주석과 구리를 녹인다 |
| 수도꼭지·주조대 | 2장 | 녹은 브론즈를 주괴로 굳힌다 |
| 파운드리 믹서 | 2장 | 1 대 3으로 섞는다. 브론즈가 나오는 유일한 곳 |
| 손 크랭크 | 3장 | 마지막에 펌프와 믹서를 **동시에** 돌린다 |
| 축·톱니바퀴 | 3장 | 크랭크의 256 SU를 두 대에 나눠 보낸다 |
| 원시 맷돌 | 3장 | 부스러기를 두 배로. 케이싱과 합금이 그것으로 서 있다 |
| 원시 선광기 | 3장 | **철을 씻어 주석을 찾아낸다.** 이 시대에 주석이 나오는 곳은 여기뿐이다 |
| 기계식 공기 펌프 | **6장** | 젓는 내내 가마를 가열로 붙잡아 둔다 |
| 프레스 | 5장 | **브론즈 주괴를 눌러 판으로 만든다.** 관문 4가 여기서 값을 한다 |
| 껍질 벗긴 원목 | 1장 | 케이싱이 입혀지는 몸통. 관문 3에서 한 번, 마지막에 한 번 더 |

**빠뜨릴 수 있는 칸이 없다.** 주석은 선광기를 통해서만 나오고, 구리는 풀무를 댄 숯가마 위에서만 녹고, 브론즈는 믹서에서만 나오고, 그 믹서는 펌프 없이는 돌지 못한다. 어느 한 줄을 건너뛰는 지름길을 두지 않았기 때문에, 시대의 마지막 행동이 곧 시대 전체의 요약이 된다.

#### 마지막 한 걸음은 풀무를 손에서 놓는 것

합금은 젓는 내내 가열을 요구하는데 사람에게는 손이 하나뿐이다. 풀무를 잡으면 크랭크를 못 잡고, 크랭크를 잡으면 불이 식는다.

그래서 **풀무에 축을 꿰어 크랭크에 물린다.** 6장이 새로 요구하는 기계는 이것 하나뿐이고, 그 하나가 이 시대의 결론이다 — 손으로 밟던 것을 기계가 밟게 만드는 것. 다음 시대가 통째로 그 이야기다.

크랭크 하나가 펌프(128 SU)와 믹서(128 SU)를 정확히 채우고 남는 것이 0이라는 사실이 그 경계를 숫자로 말해 준다. **손으로 갈 수 있는 데까지가 여기다.**
...
| 원시 제련 | 점토 성형 3종, 노천 가마(`firing`), 숯가마(`charring`), 파운드리 베이슨·뚜껑·수도꼭지·주조대(`melting` / `casting`), 열원 3단계 |
| 브론즈 라인 | 녹은 금속 5종(아연·주석·구리·철·브론즈, 전부 버킷 없음), 파운드리 믹서(`alloying`, 가열 요구), 풀무, 기계식 공기 펌프 |
...
**녹은 금속에는 버킷이 없다.** 다섯 종 전부 버킷 아이템을 등록하지 않는다. 하나라도 주면 수도꼭지와 주조대를 건너뛸 수 있고, 그러면 파운드리가 네 칸짜리 라인이 아니라 그릇 하나가 된다. Create도 포션과 차를 같은 이유로 버킷 없이 두고 있다.
...
브론즈 주괴는 이제 **판으로 눌려 케이싱이 된다.** 그 앞까지가 이 시대이고, 케이싱 이후 — 브론즈 축·톱니바퀴·기어박스 — 가 비어 있다. `bronze_gearbox_component`가 그때 쓰인다.
...
- **전 구간 플레이.** 조합으로 이어지는 것은 확인했고 파운드리 라인은 실제로 굴려 보는 중이지만, 처음부터 프레스까지 한 번에 이어 본 적은 없다. 굴려 보고서야 나온 것들이 있다 — 주조대가 틀이 요구하는 양을 넘겨 받던 것, 수도꼭지가 엉뚱한 축으로 돌아가 있던 것. **손으로 읽어서는 안 나오는 종류의 것들이므로 남은 구간도 같은 방식으로 봐야 한다.**
