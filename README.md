...
주석은 232°C — 이 팩에서 가장 낮다. **모닥불로 녹는다.** 그래서 주석을 손에 넣는 순간 이미 부을 수 있고, 브론즈 앞에 새 열원이 서지 않는다.
...
**철은 반대다.** 1538°C이므로 그 제련 레시피는 블레이즈 버너를 요구하고, 이 시대에는 돌지 않는다. 씻어 모은 철 정광은 다음 시대까지 쌓인다 — 이 시대에 선광기를 돌리는 이유는 철이 아니라 그 옆에서 나오는 주석이다. 바닐라 화로는 그대로이므로 **철 자체가 막히는 것은 아니다.** 파운드리 쪽이 더 나은 길일 뿐이고, 그 길이 아직 안 열렸을 뿐이다.
...
**녹는점이 대체로 관문이다.** 주석과 아연이 이 시대에 있는 것은 둘 다 모닥불에서 녹기 때문이고, 철이 다음 시대인 것은 그렇지 않기 때문이다.

**구리 하나만 예외로 뒀다.** 실제로는 1085°C라 블레이즈 버너 몫이지만, 그러면 브론즈 앞에 네더가 서고 이 시대의 끝이 사라진다. 아연을 약한 불에 둔 것과 정확히 같은 이유이며, 예외는 이 하나뿐이다.

숯불에 풀무를 대면 구리가 녹는 것은 실제 청동기 시대가 한 일이기도 하다. 다만 이 팩의 등급 사다리에는 "숯불"이라는 칸이 없어서, 규칙을 늘리는 대신 예외 하나를 적어 두는 쪽을 택했다.
...
꺼진 가마는 아무리 펌프질해도 꺼진 채다 — 없는 불에 바람을 넣을 수는 없다.

주둥이가 가마를 향하도록 놓이며, 가마를 부수면 함께 떨어진다.
...
숯가마 자신의 숯 굽기도 같은 값을 읽으므로, 풀무를 밟으면 가마 스스로도 한 칸 더 뜨거워진다. 열원이 블록 하나에 적혀 있고 모두가 같은 자리를 읽는다.

**아직 이 가열을 요구하는 레시피는 없다.** 지금 `heated`를 요구하는 것은 철 제련뿐이고 철은 다음 시대의 몫이다. 구리를 예외로 둔 자리를 이걸로 대신할 수는 있지만 — 그러면 구리 한 번 녹이는 200틱 동안 사람이 열 번 펌프질해야 한다. 예외를 지우는 대신 그 대가를 받을지는 정해야 한다.
...
6장은 새 기계를 요구하지 않는다. **이미 세운 것을 전부 한 번씩 다시 쓴다** — 선광기, 파운드리, 크랭크. 그래서 마지막 장이 시대를 요약한다.
...
주석은 철 원석을 씻어야만 나오고(25%), 브론즈는 믹서에서만 나온다. 그래서 진입 조건은 **어떤 아이템 하나를 만드는 것이 아니라 라인 전체를 세우는 것**이다 — 선광기를 물에 담가 돌리고, 파운드리로 붓고, 크랭크로 믹서를 돌린다. 이 시대가 만든 것 전부가 마지막에 한 번 같이 쓰인다.
...
`molten_iron`은 만들어 뒀지만 **이 시대에는 부을 수 없다.** 제련 레시피가 블레이즈 버너를 요구하기 때문이며, 철 정광은 그때까지 쌓인다.
