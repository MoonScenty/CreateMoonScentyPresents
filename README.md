...
믹서는 32 RPM에서 128 SU를 먹고 손 크랭크는 256 SU를 낸다. **크랭크 하나가 믹서 하나**이므로, 마지막 합금도 사람이 서서 돌린다.
...
| 파운드리 믹서 | 합금. 32 RPM에서 128 SU를 먹으므로 **크랭크 하나가 믹서 하나** | 회전 동력 |
...
6장은 새 기계를 요구하지 않는다. **이미 세운 것을 전부 한 번씩 다시 쓴다** — 선광기, 숯가마, 풀무, 파운드리, 크랭크. 그래서 마지막 장이 시대를 요약한다.
...
| 파운드리 믹서 | `foundry_mixer` | 합금. 아직 `alloying` 레시피가 없다 | 내화벽돌 ×4 + Wooden Shaft ×1 + 철괴 ×1 + Wooden Bearing ×1 |
